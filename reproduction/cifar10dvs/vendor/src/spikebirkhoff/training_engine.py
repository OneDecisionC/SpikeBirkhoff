# Adapted from STAtten and the timm/Spike-Driven-Transformer training scripts.
# Copyright (c) 2025 Donghyun Lee; Copyright 2019 Ross Wightman.
# See THIRD_PARTY_NOTICES.md and LICENSES for upstream notices and licenses.
# Modifications (c) 2026 Zhiqi Cai: packaged imports, unified controls,
# backend selection, and restriction to the three paper datasets.
import argparse
import time
import yaml
import os
...
from spikebirkhoff import model, datasets as dvs_utils
...
parser.add_argument("-data-dir", "--data-dir", metavar="DIR", default="", help="path to dataset",)
...
parser.add_argument("--train-split-path",type=str,default=None,metavar="N")
parser.add_argument("--split-manifest", type=str, default=None)
parser.add_argument("--model", default="sdt", type=str, metavar="MODEL", help='Name of model to train')
parser.add_argument("--pooling-stat", default="1111", type=str, help="pooling layers in embedding layer")
...
parser.add_argument("--dim", default=512, type=int, help="embedding width")
...
parser.add_argument("--chunk-size", type=int, default=2, metavar="N")
...
parser.add_argument("--epoch-repeats", type=float, default=0.0, metavar="N", help="epoch repeat multiplier (number of times to repeat dataset epoch per train epoch).")
...
parser.add_argument("--output", default=None, type=str, metavar="PATH", help="path to output folder (default: none, current dir)")
...
parser.add_argument(
    ...
    type=str,
    default="standard",
    choices=["standard", "birkhoff", "unconstrained", "row_projected", "ds_projected", "fixed_A0", "fixed_I", "fixed_J"],
    help="block-level residual routing control",
)
parser.add_argument("--n-streams", type=int, default=1, help="number of residual streams")
parser.add_argument(
    "--routing-init-scale",
    type=float,
    default=1e-2,
    help="deterministic offset scale used to break multi-stream symmetry",
)
parser.add_argument(
    "--collect-diagnostics",
    "--collect-routing-diagnostics",
    dest="collect_diagnostics",
    action="store_true",
    default=False,
    help="collect small routing and activation diagnostics on logged batches",
)
parser.add_argument(
    "--no-collect-diagnostics",
    dest="collect_diagnostics",
    action="store_false",
    help="disable diagnostics even when enabled by a YAML config",
)
parser.add_argument(
    "--max-train-batches",
    type=int,
    default=0,
    help="limit batches per training epoch for smoke tests; 0 uses all batches",
)
parser.add_argument(
    "--max-val-batches",
    type=int,
    default=0,
    help="limit validation batches for smoke tests; 0 uses all batches",
)
...
def _normalize_config_defaults(cfg):
    if cfg is None:
        return {}
    ...
    legacy_ignored = {"use_conv_as_linear"}
    normalized = {}
    sources = {}
    for raw_key, value in cfg.items():
        if not isinstance(raw_key, str):
            raise ValueError("all YAML config keys must be strings")
        destination = raw_key.replace("-", "_")
        if destination in normalized:
            raise ValueError(
                f"duplicate YAML keys map to {destination!r}: "
                f"{sources[destination]!r} and {raw_key!r}"
            )
        normalized[destination] = value
        sources[destination] = raw_key

    unknown = sorted(set(normalized) - known_destinations - legacy_ignored)
    if unknown:
        raise ValueError("unknown YAML config keys: " + ", ".join(unknown))
    for key in sorted(set(normalized) & legacy_ignored):
        _logger.warning("Ignoring legacy, unused YAML key: %s", key)
...
def _parse_args():
    ...
    if args_config.config:
        with open(args_config.config, "r") as f:
            cfg = _normalize_config_defaults(yaml.safe_load(f))
    ...
    # Several upstream YAML files omit patch_size, while MS_SPS requires a
    # concrete value. Its actual spatial stride is determined by the enabled
    # pooling stages, so infer the matching patch size when it is omitted.
    if args.patch_size is None:
        args.patch_size = 2 ** str(args.pooling_stat).count("1")
    if args.n_streams < 1:
        parser.error("--n-streams must be at least 1")
    if args.routing == "standard" and args.n_streams != 1:
        parser.error("standard routing requires --n-streams 1")
...
def _native_amp_backward_for_diagnostics(
    loss,
    optimizer,
    loss_scaler,
    clip_grad=None,
    ...
    """Backward and unscale native-AMP gradients without stepping yet.

    The timm ``NativeScaler`` convenience call performs backward and optimizer
    step together, which is too late for trustworthy gradient diagnostics.
    Return the backward scale as retained non-parameter gradients are not
    covered by ``GradScaler.unscale_``.
    """
    scaler = loss_scaler._scaler
    state_gradient_scale = float(scaler.get_scale())
    scaler.scale(loss).backward(create_graph=create_graph)
    scaler.unscale_(optimizer)
    if clip_grad is not None:
        if parameters is None:
            raise ValueError("parameters are required when clipping gradients")
        dispatch_clip_grad(parameters, value=clip_grad, mode=clip_mode)
    return state_gradient_scale
...
def _block_gradient_norms(model):
    core_model = _unwrap_model(model)
    blocks = getattr(core_model, "block", [])
    return [_module_grad_norm(block) for block in blocks]


def _write_diagnostic_record(output_dir, record):
    if not output_dir:
        return
    path = os.path.join(output_dir, "routing_diagnostics.jsonl")
    with open(path, "a", encoding="utf-8") as stream:
        stream.write(json.dumps(record, sort_keys=True) + "\n")


def _collect_diagnostic_record(
    model,
    hook,
    epoch,
    batch_idx,
    grad_norm,
    state_gradient_scale=1.0,
):
    firing_rates = {}
    if hook:
        firing_rates = {
            ...
            if torch.is_tensor(value) and "lif" in name.lower()
        }
    core_model = _unwrap_model(model)
    routing = (
        core_model.routing_diagnostics(
            state_gradient_scale=state_gradient_scale
        )
        if hasattr(core_model, "routing_diagnostics")
        else {}
    )
    return {
        "epoch": int(epoch),
        ...
        "block_gradient_norms": _block_gradient_norms(model),
        "peak_memory_mib": float(torch.cuda.max_memory_allocated() / 2**20),
        "mean_firing_rate": (
            float(sum(firing_rates.values()) / len(firing_rates))
            if firing_rates
            else None
        ),
...
def main():
    ...
    if args.dataset not in ("torch/cifar10", "torch/cifar100", "ncaltech101", "cifar10-dvs"):
    ...
    model = create_model(
        ...
        routing_init_scale=args.routing_init_scale,
    ...
    if args.local_rank == 0:
        ...
        _logger.info(
            str(
                # Do not run a CPU dummy forward here: this model explicitly
                # uses SpikingJelly's CuPy backend and is moved to CUDA below.
    ...
    if args.dataset == "cifar10-dvs":
        from rental_data import build_datasets
        dataset_train, dataset_eval = build_datasets(args.data_dir, args.split_manifest)
    ...
    if args.dvs_aug or args.dvs_trival_aug:
        raise ValueError("Legacy DVS augmentations are outside the paper configurations")
    mixup_fn = None
...
def train_one_epoch(
    ...
    if args.max_train_batches > 0:
        last_idx = min(last_idx, args.max_train_batches - 1)
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        collect_diagnostics = args.collect_diagnostics and (
            last_batch or batch_idx % args.log_interval == 0
        )
        diagnostic_hook = {} if collect_diagnostics else None
        with amp_autocast():
            output, diagnostic_hook = model(input, hook=diagnostic_hook)
            if args.TET:
                loss = TET_loss(
        ...
        if (
            ...
            and isinstance(loss_scaler, NativeScaler)
        ):
            state_gradient_scale = _native_amp_backward_for_diagnostics(
                loss,
                optimizer,
                loss_scaler,
            ...
            diagnostic_record = _collect_diagnostic_record(
                model,
                diagnostic_hook,
                epoch,
                batch_idx,
        ...
        # ApexScaler does not expose the same split backward/unscale/step API.
        # Preserve its existing path while still logging after its call.
        if collect_diagnostics and diagnostic_record is None:
            grad_norm = _global_grad_norm(model)
            diagnostic_record = _collect_diagnostic_record(
                model,
                diagnostic_hook,
        ...
        if collect_diagnostics and args.local_rank == 0:
            _write_diagnostic_record(output_dir, diagnostic_record)
            ...
            if args.local_rank == 0:
                _logger.info(
                    ...
                    "Data: {data_time.val:.3f} ({data_time.avg:.3f})".format(
                        ...
                        batch_idx,
                        len(loader),
                        100.0 * batch_idx / max(1, last_idx),
                        loss=losses_m,
    ...
    samples_per_second = sample_number / elapsed
    peak_memory_mib = torch.cuda.max_memory_allocated() / 2**20
    if args.local_rank == 0:
        _logger.info(f"samples / s = {samples_per_second: .3f}")
        _logger.info(f"peak allocated memory = {peak_memory_mib: .1f} MiB")
    metrics = OrderedDict(
        [
            ("loss", losses_m.avg),
    ...
    if args.collect_diagnostics:
        metrics["gradient_norm"] = grad_norm_m.avg
    return metrics
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=""):
    ...
    if args.max_val_batches > 0:
        last_idx = min(last_idx, args.max_val_batches - 1)
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            if batch_idx > last_idx:
                break
