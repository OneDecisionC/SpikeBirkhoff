...
class MS_MLP_Conv(nn.Module):
    def __init__(
        ...
        if spike_mode == "lif":
            ...
            self.fc1_lif = MultiStepParametricLIFNode(init_tau=2.0, detach_reset=True, backend=backend)
        ...
        if spike_mode == "lif":
            ...
            self.fc2_lif = MultiStepParametricLIFNode(init_tau=2.0, detach_reset=True, backend=backend)
...
class MS_SSA_Conv(nn.Module):
    def __init__(
        ...
        self.talking_heads_lif = MultiStepLIFNode(tau=2.0, v_threshold=0.5, detach_reset=True, backend=backend)
...
class MS_Block_Conv(nn.Module):
    def __init__(
        ...
        # Retained for checkpoint/runtime parity: archived forward never uses drop_path.
