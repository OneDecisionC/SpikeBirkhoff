"""Single-layer mixer interventions with full validation and native membrane probes."""
import argparse
...
def worker(args, plan):
    ...
    variants = ('B',) + tuple(f'layer_{index}_{mode}' for index in range(len(model.routing_layers))
                              for mode in ('A0', 'I'))
    ...
    def intervene(variant):
        ...
        if variant != 'B':
            ...
            if mode == 'A0':
                router.mixing_logits.copy_(initial[int(target)].to(router.mixing_logits))
            else:
    ...
    print('LAYERWISE_COMPLETE ' + args.job, flush=True)
...
def main():
    ...
    save(args.output / 'layerwise_plan.json', {'prior_plan': str(args.plan), 'prior_plan_sha256': digest(args.plan),
        'script_sha256': digest(__file__), 'records': records, 'gpus': gpus, 'smoke_only': args.smoke,
        'policy': 'All individual layers A0/I, including last-layer negative control; full validation and frozen probes; no test access.'})
