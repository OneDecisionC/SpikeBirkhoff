"""Observe the native CuPy LIF pre-fire buffer on frozen validation probes."""
import argparse
...
def worker(args, plan):
    ...
    if plan['dataset'] == 'cifar100':
        ...
        create_loader(dataset, input_size=data_cfg['input_size'], batch_size=8, is_training=False,
        ...
        for index in previous['probe_indices']:
    ...
    save(args.output / (args.job + '.json'), {'server': plan['server'], 'seed': cfg['seed'],
        ...
        'partition': 'fixed_validation_probe', 'smoke_only': args.smoke,
        ...
        'method': 'Unchanged native CuPy forward; capture h_seq through dummy context, no backward.',
    ...
    print('MEMBRANE_COMPLETE ' + args.job, flush=True)
...
def main():
    ...
    save(args.output / 'membrane_plan.json', {'prior_plan': str(args.plan), 'prior_plan_sha256': digest(args.plan),
        'script_sha256': digest(__file__), 'records': records, 'gpus': gpus, 'smoke_only': args.smoke,
        'policy': 'Same frozen validation probes and four interventions; no training/test access.'})
    ...
    def lane(index):
        for row in records[index::len(gpus)]:
            ...
            command = [sys.executable, __file__, '--plan', str(args.plan), '--output', str(args.output), '--job', row['name']]
            if args.smoke:
